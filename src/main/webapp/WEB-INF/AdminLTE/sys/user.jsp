  ...
  <body class="hold-transition skin-blue sidebar-mini animsition ">
	...
	$(function() {
		var $data_table = jq.table({
			url:"/sys/user/list",
			 fields:["id","name","email","mobile","birthday","updatedTime"],
	        "columnDefs": [ {
	            "targets": 4,
	            "createdCell": function (td, cellData, rowData, row, col) {
	            	if(cellData)
	                	$(td).text(cellData.substr(0,10));
	            }
	          }]
			});
		...
		// 初始化zTree数据	    
	    jq.ajax("/sys/role/list-all", {}, function(data){
			data = data.object;
			// console.log(data);
			// 初始化数据
			var zNodes = [];
		...
		var $refresh_resource = function(){
			var user = $data_table.rows( { selected: true } ).data().toArray()[0];
			// TODO 查询已有资源
   			jq.ajax("/sys/user-role/query", {'accountId':user.id}, function(data){
		...
		$('#ztree-cell-update').click(function(){
			// 获取选中的tree
	  		...
	  		var roleIds = '';
	  		for (var i=0, l=nodes.length; i < l; i++) {
	  			roleIds += nodes[i].id+',';// ','区分
			}
			var user = $data_table.rows( { selected: true } ).data().toArray()[0];
			jq.ajax("/sys/user-role/update", {'accountId':user.id,'roleIds':roleIds}, function(data){
		...
		var edit = mdl.editor({
			...
			fields:[
				    ...
				    {name:'mobile',type:'text',label:'手机号'},
				    {name:'birthday',type:'text',label:'生日',placeholder:'生日（yyyy-MM-dd）'}
				    ]
		...
		$show_create.click(function(){
			edit.showModal({action:'/sys/user/create',title:'创建',value:false});
		});
		
		$show_update.click(function(){
	    	var selected = $data_table.rows( { selected: true } ).data().toArray()[0];
			edit.showModal({action:'/sys/user/update',title:'修改',value:selected});
		});
		...
		$show_delete.click(function(){
	    	...
	    	mdl.messager.confirm('提示','确认要删除选中的'+selected.count()+'项？',function(){
	    		...
	    		jq.ajax('/sys/user/delete',{'ids':ids.toString()},function(data){
	    			if(data.code){
